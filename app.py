...
st.markdown(
    ...
    <style>
        ...
        .metric-card {{
            background-color: {selected_theme["bg_card"]};
  ...
  with st.spinner("Processing 3D file..."):
    try:
      ...
      with tab2:
        ...
        with st.expander("🔍 รายละเอียดทางเทคนิคเพิ่มเติม (Advanced Specs)"):
      ...
      with tab3:
        ...
        material = st.selectbox("เลือกประเภทเส้นพลาสติก", ["PLA", "ABS", "PETG"])
        density_map = {"PLA": 1.24, "ABS": 1.04, "PETG": 1.27}
        infill = st.slider("เปอร์เซ็นต์ Infill (%)", 10, 100, 20)
        price_per_gram = st.number_input("ราคาเส้นพลาสติก (บาท/กรัม)", value=0.8)

        # คำนวณน้ำหนักและราคาประมาณการ
        estimated_weight = volume_cu_cm * density_map[material] * (infill / 100)
